...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        if(queries > done_queries)  //if lines are more than the threads we need to create more threads for the rest of the lines
        {
            ...
            while(done != num_threads){}  //wait until all threads are done
            ...
            {
                lines_done++;
                if(lines_done > (num_threads*times))  //start creating threads after passed n * times lines (if this is done 2 times then there are 2 * num_threads made)
                {
                    if(curr_threads <= num_threads) //no more than n threads
                    {
                        string* query_copy = new string(query);
